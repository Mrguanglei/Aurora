...
class ModelRegistry:
    ...
    def _initialize_models(self):
        ...
        # Doubao (ByteDance 豆包 - Chinese LLM)
        if config.DOUBAO_API_KEY:
            self.register(Model(
                id="doubao/doubao-seed-1-6-251015",
                name="Doubao Seed 1.6",
                provider=ModelProvider.DOUBAO,
                aliases=["doubao", "Doubao", "doubao-seed"],
                context_window=200_000,
                capabilities=[
                    ModelCapability.CHAT,
                    ModelCapability.FUNCTION_CALLING,
                ...
                pricing=ModelPricing(
                    input_cost_per_million_tokens=0.10,  # 示例价格，请根据实际调整
                ...
                enabled=True,
                config=ModelConfig(
                    api_base=config.DOUBAO_API_BASE,
                )
            ))
