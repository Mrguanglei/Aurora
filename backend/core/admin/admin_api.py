...
@router.post("/aurora-agents/install-user/{account_id}")
async def admin_install_aurora_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install Aurora agent for a specific user."""
    logger.debug(f"Admin installing Aurora agent for user: {account_id}")
    
    service = SunaDefaultAgentService()
    agent_id = await service.install_aurora_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Aurora agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Aurora agent for user {account_id}"
        )
