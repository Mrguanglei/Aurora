...
import AuroraSymbolIcon from '@/assets/brand/aurora-symbol.svg';
...
/**
 * Check if a model ID corresponds to an Aurora mode (Basic or Advanced)
 */
export function isAuroraMode(modelId: string): boolean {
  ...
  // Legacy: Aurora Basic (Haiku 4.5)
  if (modelId.includes('claude-haiku-4-5') || modelId.includes('heol2zyy5v48')) {
    return true;
  }
  // Legacy: Aurora Advanced Mode (Sonnet 4.5)
...
export function getModelProvider(modelId: string): ModelProvider {
  // Check for Aurora modes first
  if (isAuroraMode(modelId)) {
    return 'kortix';
  }
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    aurora: AuroraSymbolIcon, // Aurora modes use the Aurora symbol
