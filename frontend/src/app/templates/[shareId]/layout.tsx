...
export async function generateMetadata({ params }: { params: Promise<{ shareId: string }> }): Promise<Metadata> {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8011/v1'}/templates/public/${templateId}`);
    ...
    const title = `${template.name} - AI Agent Template | Aurora`;
    ...
    return {
      title: 'AI Agent Template | Aurora',
